...
import { Box, Bird, Cat, Rabbit, Users, Code2, Wand2, Hammer, FolderOpen, ChevronUp, FileJson, History, Play, Pause, Info, Wrench, Loader2, Dog, Turtle, Squirrel, Fish, Crown, Zap, Shield, Flame, Hexagon } from 'lucide-react';
...
interface UIOverlayProps {
  ...
  onDismantle: () => void;
  onRebuild: (type: 'Eagle' | 'Cat' | 'Rabbit' | 'Twins' | 'Dog' | 'Turtle' | 'Squirrel' | 'Fish' | 'Elephant' | 'Lion' | 'Snake' | 'Bear' | 'Dinosaur') => void;
  onNewScene: (type: 'Eagle' | 'Cat' | 'Rabbit' | 'Dog' | 'Turtle' | 'Squirrel' | 'Fish' | 'Elephant' | 'Lion' | 'Snake' | 'Bear' | 'Dinosaur') => void;
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
        ...
        <div className="pointer-events-auto flex flex-col gap-2">
            ...
            >
                ...
                <DropdownItem onClick={() => onNewScene('Dog')} icon={<Dog size={16}/>} label="Dog" />
                <DropdownItem onClick={() => onNewScene('Rabbit')} icon={<Rabbit size={16}/>} label="Rabbit" />
                <DropdownItem onClick={() => onNewScene('Turtle')} icon={<Turtle size={16}/>} label="Turtle" />
                <DropdownItem onClick={() => onNewScene('Squirrel')} icon={<Squirrel size={16}/>} label="Squirrel" />
                <DropdownItem onClick={() => onNewScene('Fish')} icon={<Fish size={16}/>} label="Fish" />
                <DropdownItem onClick={() => onNewScene('Elephant')} icon={<Hexagon size={16}/>} label="Elephant" />
                <DropdownItem onClick={() => onNewScene('Lion')} icon={<Crown size={16}/>} label="Lion" />
                <DropdownItem onClick={() => onNewScene('Snake')} icon={<Zap size={16}/>} label="Snake" />
                <DropdownItem onClick={() => onNewScene('Bear')} icon={<Shield size={16}/>} label="Bear" />
                <DropdownItem onClick={() => onNewScene('Dinosaur')} icon={<Flame size={16}/>} label="Dinosaur" />
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 animate-in fade-in zoom-in duration-300">
              <div className="bg-white/90 backdrop-blur-md border-2 border-indigo-100 px-8 py-6 rounded-3xl shadow-2xl flex flex-col items-center gap-4 min-w-[280px]">
                  ...
                  <div className="text-center">
                      <h3 className="text-lg font-extrabold text-slate-800">Building...</h3>
                     ...
                     >
                        ...
                        {isEagle && (
                            <>
                                ...
                                <DropdownItem onClick={() => onRebuild('Dog')} icon={<Dog size={18}/>} label="Dog" />
                                <DropdownItem onClick={() => onRebuild('Rabbit')} icon={<Rabbit size={18}/>} label="Rabbit" />
                                <DropdownItem onClick={() => onRebuild('Turtle')} icon={<Turtle size={18}/>} label="Turtle" />
                                <DropdownItem onClick={() => onRebuild('Squirrel')} icon={<Squirrel size={18}/>} label="Squirrel" />
                                <DropdownItem onClick={() => onRebuild('Fish')} icon={<Fish size={18}/>} label="Fish" />
                                <DropdownItem onClick={() => onRebuild('Elephant')} icon={<Hexagon size={18}/>} label="Elephant" />
                                <DropdownItem onClick={() => onRebuild('Lion')} icon={<Crown size={18}/>} label="Lion" />
                                <DropdownItem onClick={() => onRebuild('Snake')} icon={<Zap size={18}/>} label="Snake" />
                                <DropdownItem onClick={() => onRebuild('Bear')} icon={<Shield size={18}/>} label="Bear" />
                                <DropdownItem onClick={() => onRebuild('Dinosaur')} icon={<Flame size={18}/>} label="Dinosaur" />
